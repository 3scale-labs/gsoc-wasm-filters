...
impl CacheFilter {
    fn report_to_singleton(&self, queue_id: Option<u32>, request_data: &ThreescaleData) -> bool {
        match queue_id {
            ...
            None =>
            {
                info!("ctxt {}: Reporting to singleton failed: Queue id not provided", self.context_id);
                return true;
            }
        }
        false
    ...
    fn is_rate_limited(&mut self, request_data: &ThreescaleData, app: &mut Application, _current_time: &SystemTime) -> bool {
